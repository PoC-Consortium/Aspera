...
void calculate_deadline(CalcDeadlineRequest *req) {
    ...
    // XOR with final
    for (int i = 0; i < NONCE_SIZE; i++) gendata[i] ^= (final[i % 32]);
...
void calculate_deadlines_sse4(CalcDeadlineRequest **reqs) {
    ...
    for (int i = NONCE_SIZE; i > 0; i -= HASH_SIZE) {
        ...
        sse4_mshabal_close(&x, 0, 0, 0, 0, 0, &gendata[0][i - HASH_SIZE],
                           &gendata[1][i - HASH_SIZE], &gendata[2][i - HASH_SIZE],
                           &gendata[3][i - HASH_SIZE]);
    }
    ...
    for (int i = 0; i < NONCE_SIZE; i++)
        for (int j = 0; j < SSE4_PARALLEL; j++) gendata[j][i] ^= (finals[j][i % 32]);

    mshabal_context deadline_sc;
    sse4_mshabal_init(&deadline_sc, 256);
    sse4_mshabal(&deadline_sc, reqs[0]->gen_sig, reqs[1]->gen_sig, reqs[2]->gen_sig,
                 reqs[3]->gen_sig, HASH_SIZE);
...
void calculate_deadlines_avx2(CalcDeadlineRequest **reqs) {
    ...
    mshabal256_close(&x, (uint32_t *)finals[0], (uint32_t *)finals[1], (uint32_t *)finals[2],
                     (uint32_t *)finals[3], (uint32_t *)finals[4], (uint32_t *)finals[5],
                     (uint32_t *)finals[6], (uint32_t *)finals[7]);

    // XOR with final
    for (int i = 0; i < NONCE_SIZE; i++)
        for (int j = 0; j < AVX2_PARALLEL; j++) gendata[j][i] ^= (finals[j][i % 32]);

    mshabal256_context deadline_sc;
    mshabal256_init(&deadline_sc);
    mshabal256(&deadline_sc, reqs[0]->gen_sig, reqs[1]->gen_sig, reqs[2]->gen_sig, reqs[3]->gen_sig,
               reqs[4]->gen_sig, reqs[5]->gen_sig, reqs[6]->gen_sig, reqs[7]->gen_sig, HASH_SIZE);
    ...
    mshabal256_close(&deadline_sc, (uint32_t *)finals2[0], (uint32_t *)finals2[1],
                     (uint32_t *)finals2[2], (uint32_t *)finals2[3], (uint32_t *)finals2[4],
                     (uint32_t *)finals2[5], (uint32_t *)finals2[6], (uint32_t *)finals2[7]);
