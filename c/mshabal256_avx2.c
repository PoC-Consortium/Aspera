...
static void mshabal256_compress(mshabal256_context *sc, const uint8_t *buf0, const uint8_t *buf1,
                                const uint8_t *buf2, const uint8_t *buf3, const uint8_t *buf4,
                                const uint8_t *buf5, const uint8_t *buf6, const uint8_t *buf7,
                                size_t num) {
    union {
        uint32_t words[128];
        __m256i data[16];
...
#define M(i) _mm256_load_si256(u.data + (i))
    ...
    while (num--) {
        ...
        for (uint8_t j = 0; j < 16; j++) B[j] = _mm256_add_epi32(B[j], M(j));
...
void mshabal256_init(mshabal256_context *sc) {
    ...
    mshabal256_compress(sc, sc->buf0, sc->buf1, sc->buf2, sc->buf3, sc->buf4, sc->buf5, sc->buf6,
                        sc->buf7, 1);
    for (uint8_t u = 0; u < 16; u++) {
        uint8_t idx = u * 4;
        sc->buf0[idx] = 272 + u;
...
void mshabal256(mshabal256_context *sc, const void *data0, const void *data1, const void *data2,
    ...
    if (ptr != 0) {
        ...
        if (clen > len) {
            ...
            mshabal256_compress(sc, sc->buf0, sc->buf1, sc->buf2, sc->buf3, sc->buf4, sc->buf5,
                                sc->buf6, sc->buf7, 1);
