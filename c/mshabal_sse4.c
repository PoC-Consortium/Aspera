...
static void sse4_mshabal_compress(mshabal_context *sc, const unsigned char *buf0,
...
void sse4_mshabal_close(mshabal_context *sc, unsigned ub0, unsigned ub1, unsigned ub2, unsigned ub3,
    ...
    if (dst0 != NULL) {
        ...
        out = dst0;
        for (z = 0; z < out_size_w32; z++) out[z] = sc->state[off + (z << 2) + 0];
    }
    if (dst1 != NULL) {
        u32 *out;

        out = dst1;
        for (z = 0; z < out_size_w32; z++) out[z] = sc->state[off + (z << 2) + 1];
    }
    if (dst2 != NULL) {
        u32 *out;

        out = dst2;
        for (z = 0; z < out_size_w32; z++) out[z] = sc->state[off + (z << 2) + 2];
    }
    if (dst3 != NULL) {
        u32 *out;

        out = dst3;
        for (z = 0; z < out_size_w32; z++) out[z] = sc->state[off + (z << 2) + 3];
    }
}
